...
{
    SceneTriangle trangles[NUM_FACES];
}
...
@property (nonatomic,strong) AGLVertexAttribArrayBuffer *extraBuffer;
...
- (void)updateNormals
{
    if (self.shouldUseFaceNormals) {
        SceneTrianglesUpdateFaceNormals(trangles);
    }else{
        SceneTrianglesUpdateVertexNormals(trangles);
    }
    
    [self.vertexBuffer reinitWithAttribStride:sizeof(SceneVertex)
                             numberOfVertices:sizeof(trangles)/sizeof(SceneVertex)
                                        bytes:trangles];
}
...
- (void)viewDidLoad {
    ...
    self.baseEffect.light0.diffuseColor = GLKVector4Make(0.7f, 0.7f, 0.7f, 1.0f);  // RGB
    self.baseEffect.light0.position = GLKVector4Make(1.0f, 1.0f, 0.5f, 0.5f);
    
    self.extraEffect = [[GLKBaseEffect alloc] init];
    self.extraEffect.useConstantColor = GL_TRUE;
    self.extraEffect.constantColor = GLKVector4Make(0.0f, 1.0f, 0.0f, 1.0f); // RGB
    ...
    trangles[0] = SceneTriangleMake(vertexA, vertexB, vertexD);
    trangles[1] = SceneTriangleMake(vertexB, vertexC, vertexF);
    trangles[2] = SceneTriangleMake(vertexD, vertexB, vertexE);
    trangles[3] = SceneTriangleMake(vertexE, vertexB, vertexF);
    trangles[4] = SceneTriangleMake(vertexD, vertexE, vertexH);
    trangles[5] = SceneTriangleMake(vertexE, vertexF, vertexH);
    trangles[6] = SceneTriangleMake(vertexG, vertexD, vertexH);
    trangles[7] = SceneTriangleMake(vertexH, vertexF, vertexI);
    ...
    self.extraBuffer = [[AGLVertexAttribArrayBuffer alloc]
                        initWithAttribStride:sizeof(SceneVertex)
                        numberOfVertices:0
                        bytes:NULL
                        usage:GL_DYNAMIC_DRAW];
...
- (void)drawNormals
{
    ...
    SceneTrianglesNormalLinesUpdate(trangles,
...
- (void)setCenterVertexHeight:(GLfloat)aValue
{
   ...
   trangles[2] = SceneTriangleMake(vertexD, vertexB, newVertexE);
   trangles[3] = SceneTriangleMake(newVertexE, vertexB, vertexF);
   trangles[4] = SceneTriangleMake(vertexD, newVertexE, vertexH);
   trangles[5] = SceneTriangleMake(newVertexE, vertexF, vertexH);
